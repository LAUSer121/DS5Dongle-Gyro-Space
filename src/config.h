...
struct __attribute__((packed)) Config_body {
    ...
    // (The previously-dormant gyro_space_mode from master is superseded by this.)
    ...
    // Battery display refinement (v1.20.x, portal-selectable).
    ...
    // (A voltage-blend option was tried but removed: the DualSense Bluetooth
    // report carries no voltage, and the factory-test command that could read
    // one disconnects retail controllers and collides with the portal's 0x81
    // command channel.)
